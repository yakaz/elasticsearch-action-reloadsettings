 ...
 *
 */
public class ESInternalSettingsPerparer {

    public static URL getConfigurationURL(Settings pSettings) {
        ...
        if (useSystemProperties) {
            ...
            if (System.getProperty("es.default.config") != null) {
                loadFromEnv = true;
                return environment.resolveConfig(System.getProperty("es.default.config"));
            }
            ...
            if (System.getProperty("elasticsearch.config") != null) {
                loadFromEnv = false;
                return environment.resolveConfig(System.getProperty("elasticsearch.config"));
            }
        }
        if (loadFromEnv) {
            try {
                return environment.resolveConfig("elasticsearch.yml");
            ...
            try {
                return environment.resolveConfig("elasticsearch.json");
            } catch (FailedToResolveConfigException e) {
                // ignore
            }
            try {
                return environment.resolveConfig("elasticsearch.properties");
            } catch (FailedToResolveConfigException e) {
                // ignore
            }
        }
